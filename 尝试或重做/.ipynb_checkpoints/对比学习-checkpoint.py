...
class my_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if possibility > 0.5:
            ...
            while self.y_data[idx2] == y:
                idx2 = np.random.randint(0, len(self.y_data) - 1)
        extra = self.x_data[idx2]
        return x, extra, sign, y
...
class Siamese(nn.Module):
    def __init__(self, output_size):
        ...
        self.features = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Dropout(0.5),
            nn.Linear(3136, out_features=512),
            nn.Dropout(0.5),
            nn.Linear(512, out_features=16)
        )
...
def contrastive_loss(pred1, pred2, y):
    MARGIN = 5
    distance = F.pairwise_distance(pred1, pred2)
    y = y.view(-1)
    loss = (1 - y) * torch.pow(distance, 2) + y * torch.pow(torch.clamp(MARGIN - distance, min=0), 2)
    return loss.mean()
...
def train(model, train_loader, optimizer, epochs, device):
    ...
    for epoch in range(epochs):
        total_loss = 0
        for i, (x1, x2, y, _) in enumerate(train_loader):
            x1, x2, y = x1.to(device), x2.to(device), y.to(device)
            pred1 = model(x1)
            pred2 = model(x2)
            loss = contrastive_loss(pred1, pred2, y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def eval(model, device, test_loader, epochs):
    ...
    plt.ylabel('Loss')
    plt.show()
    tsne3d = TSNE(n_components=3, init='pca', perplexity=30., random_state=0, learning_rate=20)
    fig = plt.figure()
    ...
    with torch.no_grad():
        ...
        for x, _, _, y in test_loader:
            ...
            if num == 2:
                break
        plt.show()
    tsne2d = TSNE(n_components=2, init='pca', perplexity=30., random_state=0, learning_rate=20)
    with (torch.no_grad()):
        num = 0
        for x, _, _, y in test_loader:
            x, y = x.to(device), y.detach().cpu().numpy()
